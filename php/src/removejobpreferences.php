 ...
 * 
 * @param jobprefid returns the job preferences id and it is used to remove a particular
...
class RemoveJobPreferences extends Endpoint {
    ...
    protected function initialiseSQL(){
       ...
       $jobprefid=$_POST['jobprefid'];

        $sql= "DELETE FROM jobpreferences WHERE pref_id = :jobprefid";
        $this->setSQL($sql);
        $this->setSQLParams([ ':jobprefid' => $jobprefid]);
    }
    ...
    private function validateUpdateParams(){
         ...
         if (!filter_has_var(INPUT_POST,'jobprefid')) {
           throw new ClientErrorException("jobprefid parameter required", 400);
         }
     }

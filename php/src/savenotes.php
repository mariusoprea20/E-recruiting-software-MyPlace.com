...
use FirebaseJWT\Key;
/**
 * Used by the employer to save a notes on an accepted job application
 * if valid credentials are approved through JWT.
 * 
 * @param applicationid returns the id of an application and updates the note filed
 * in the application table by passing the value found in @param newnotes
 * 
 * @method POST
 * @author Marius Oprea
 * @id w20039534
 */
class SaveNotes extends Endpoint {
    ...
    protected function initialiseSQL(){
        ...
        $sql = "UPDATE application SET notes = :notes WHERE applicationId = :applicationid";
        $this->setSQL($sql);
        $this->setSQLParams([':applicationid'=>$applicationID, ':notes'=>$notes]);
    }

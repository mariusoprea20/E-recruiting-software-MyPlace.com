...
/**
 * Unsave a job from the database on jobseeker request
 * if valid credentials are approved through JWT
 * 
 * @param userid,jobid returns the id of the jobseeker and the id of a job 
 * and unsaves that job in the database
 * 
 * @method POST
 * @id w20039534
 */

class UnsaveJob extends Endpoint {


    public function __construct()
    {
       $db = new Database(DATABASE);//set te db
    ...
    protected function initialiseSQL(){
       ...
       $userid=$_POST['userid'];
       $jobid=$_POST['jobid'];
        $sql= "DELETE FROM savedjobs WHERE user_id = :userid AND job_id = :jobid";
        $this->setSQL($sql);
